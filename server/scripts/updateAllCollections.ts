...
async function updateAllCollections() {
  try {
    ...
    const projects = await db.collection('projects').updateMany(
      ...
      { $set: { isPublic: true, isFeatured: false, displayOrder: 0 } }
    );
    console.log(`✅ Projects updated: ${projects.modifiedCount}`);
    
    // Update Skills
    const skills = await db.collection('skills').updateMany(
      { isPublic: { $exists: false } },
      { $set: { isPublic: true, order: 0 } }
    );
    console.log(`💡 Skills updated: ${skills.modifiedCount}`);
    
    // Update Testimonials
    const testimonials = await db.collection('testimonials').updateMany(
      { isPublic: { $exists: false } },
      { $set: { isPublic: true } }
    );
